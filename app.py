...
def ler_aba_consultor(nome_aba):
    ...
    valores = ler_intervalo_seguro(ws, RANGE_CONSULTOR)
    if not valores:
        return pd.DataFrame(columns=["Consultor", "Etapa", "Dia", "Valor"])
...
with header_col1:
    try:
        ...
        pass
with header_col2:
    st.markdown('<div class="brand-header"><div class="brand-title">Time Aliados</div><div class="brand-subtitle">Painel otimizado para gastar menos quota da API e continuar estável durante atualizações.</div></div>', unsafe_allow_html=True)

with st.sidebar:
    st.header("Filtros")
    auto_atualizar = st.toggle("Atualização automática", value=False)
    intervalo = st.selectbox("Intervalo", [180, 300, 600], index=1, format_func=lambda x: f"{x} segundos")
...
def render_processos():
    ...
    cols = st.columns(4)
    with cols[0]:
        kpi_card("Processos", len(dfp))
    with cols[1]:
        kpi_card("Valor pipeline", formatar_moeda(dfp["valor_imovel_num"].sum()))
    with cols[2]:
        kpi_card("Financiamento", formatar_moeda(dfp["valor_financiamento_num"].sum()))
    with cols[3]:
        kpi_card("Valor previsto", formatar_moeda(dfp["valor_previsto_num"].sum()))
...
def render_roleta():
    ...
    with cols[1]:
        kpi_card("Média", f"{df_roleta['Roleta'].replace(0, pd.NA).dropna().mean() or 0:.2f}")
    with cols[2]:
