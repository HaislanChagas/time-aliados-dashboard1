...
def auto_refresh(seconds=120):
    components.html(f"<script>setTimeout(function(){{window.parent.location.reload();}}, {seconds*1000});</script>", height=0, width=0)
...
def kpi_card(title, value, subtitle=""):
    st.markdown(f'<div class="card"><div class="card-title">{title}</div><div class="card-value" style="font-size:28px;">{value}</div><div class="small-note">{subtitle}</div></div>', unsafe_allow_html=True)
...
def abrir_planilha_com_retry(sheet_id, tentativas=3, espera=2):
    ultimo_erro = None
...
def listar_abas_consultores():
    try:
        sh = abrir_planilha_com_retry(SHEET_ID)
    except Exception:
        return []
...
@st.cache_data(ttl=180)
def ler_aba_consultor(nome_aba):
    sh = abrir_planilha_com_retry(SHEET_ID)
...
def carregar_dados_produtividade():
    ...
    for aba in consultores:
        try:
            frames.append(ler_aba_consultor(aba))
        except Exception as e:
...
with header_col1:
    try:
        ...
        pass
with header_col2:
    st.markdown('<div class="brand-header"><div class="brand-title">Time Aliados</div><div class="brand-subtitle">Painel blindado com atualização automática, produtividade, processos quentes, metas, previsão e roleta de leads.</div></div>', unsafe_allow_html=True)

with st.sidebar:
    st.header("Filtros")
    auto_atualizar = st.toggle("Atualização automática", value=True)
    intervalo = st.selectbox("Intervalo", [30, 60, 120, 180], index=1, format_func=lambda x: f"{x} segundos")
...
if df.empty:
    st.warning("Nenhuma aba de consultor foi detectada no momento. O dashboard continua ativo e tentará novamente na próxima atualização automática.")
...
def render_produtividade():
    ...
    with col_b:
        ...
        st.plotly_chart(fig, use_container_width=True)
    if visao == "Equipe" and not df.empty:
...
def render_processos():
    ...
    cards = [("Processos em andamento", len(dfp), ""), ("Valor pipeline", formatar_moeda(valor_pipeline), "Soma do valor do imóvel"), ("Financiamento aprovado", formatar_moeda(valor_financiado), "Soma do valor aprovado"), ("Valor previsto", formatar_moeda(valor_previsto), "Baseado na probabilidade do status"), ("Conversão prevista", f"{(valor_previsto/valor_pipeline*100 if valor_pipeline else 0):.1f}%", "Fechamento estimado")]
...
def montar_realizado_por_consultor():
    if df.empty:
        return pd.DataFrame(columns=["consultor", "realizado_leads", "realizado_atendimento", "realizado_agendamento", "realizado_pasta_docs", "realizado_credito", "realizado_valor", "realizado_valor_previsto"])
